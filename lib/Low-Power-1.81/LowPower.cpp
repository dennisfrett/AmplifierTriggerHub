 ...
 * 1.50      Fixed compiler optimization (Arduino IDE 1.6.x branch) on BOD
 *enable function that causes the function to be over optimized. 1.40
 *Added support for ATSAMD21G18A. Library format compliant with Arduino
 *IDE 1.5.x.
 * 1.30		Added support for ATMega168, ATMega2560, ATMega1280 &
 *ATMega32U4. Tested to work with Arduino IDE 1.0.1 - 1.0.4.
 * 1.20		Remove typo error in idle method for checking whether Timer 0
 *was turned off. Remove dependecy on WProgram.h which is not required. Tested
 *to work with Arduino IDE 1.0. 1.10		Added #ifndef for
 *sleep_bod_disable() for compatibility with future Arduino IDE release. 1.00
 ...
 *			         understand the implication and result of disabling
 *module.
 *
 ...
 *			   understand the implication and result of disabling
 *module. Take note that Timer 2 is not available and USART0 is replaced with
 *USART1 on ATmega32U4.
 *
 * Argument  	Description
 * =========  	===========
 * 1. period     Duration of low power mode. Use SLEEP_FOREVER to use other wake
 ...
 *				(b) TIMER4_ON - Leave Timer 4 module in its default
 *state
 *
 * 4. timer3		Timer 3 module disable control:
 *				(a) TIMER3_OFF - Turn off Timer 3 module
 *				(b) TIMER3_ON - Leave Timer 3 module in its default
 *state
 *
 ...
 *				(b) USART1_ON - Leave USART1 module in its default
 *state
 *
 ...
 *				(b) USB_ON - Leave USB module in its default
 *state
 *******************************************************************************/
#if defined __AVR_ATmega32U4__
void LowPowerClass::idle(period_t period, adc_t adc, timer4_t timer4,
...
/*******************************************************************************
 * Name: idle
 * Description: Putting ATmega644P & ATmega1284P into idle state. Please make
 *sure you understand the implication and result of disabling module. Take note
 *that extra USART 1 compared to an ATmega328P/168.
 *
 * Argument  	Description
 * =========  	===========
 * 1. period     Duration of low power mode. Use SLEEP_FOREVER to use other wake
...
/*******************************************************************************
 * Name: idle
 * Description: Putting ATmega2560 & ATmega1280 into idle state. Please make
 *sure you understand the implication and result of disabling module. Take note
 *that extra Timer 5, 4, 3 compared to an ATmega328P/168. Also take note that
 *extra USART 3, 2, 1 compared to an ATmega328P/168.
 *
 * Argument  	Description
 * =========  	===========
 * 1. period     Duration of low power mode. Use SLEEP_FOREVER to use other wake
 ...
 *				(b) TIMER5_ON - Leave Timer 5 module in its default
 *state
 *
 ...
 *				(b) USART3_ON - Leave USART3 module in its default
 *state
 *
 * 11.usart2		USART2 module disable control:
 *				(a) USART2_OFF - Turn off USART2  module
 *				(b) USART2_ON - Leave USART2 module in its default
 *state
 *
 ...
 *			   you understand the implication and result of disabling
 *module. Take note that extra Timer 5, 4, 3 compared to an ATmega328P/168. Also
 ...
 *			   a very useful state when using the ADC to achieve best
 ...
 *WDT
 *
 * 2. adc		ADC module disable control. Turning off the ADC module
 *is basically removing the purpose of this low power mode. (a) ADC_OFF - Turn
 *off ADC module (b) ADC_ON - Leave ADC module in its default state
 *
 * 3. timer2		Timer 2 module disable control:
 *				(a) TIMER2_OFF - Turn off Timer 2 module
 *				(b) TIMER2_ON - Leave Timer 2 module in its default
 ...
 *			   the lowest current consumption state. Use this together
 *with external pin interrupt to wake up through external event triggering
 *(example: RTC clockout pin, SD card detect pin).
 *
 * Argument  	Description
 * =========  	===========
 * 1. period     Duration of low power mode. Use SLEEP_FOREVER to use other wake
 ...
 *				(b) BOD_ON - Leave BOD module in its default
 *state
 *
 *******************************************************************************/
 ...
 *			   the lowest current consumption state after power
 *down. Use this state together with an external 32.768 kHz crystal (but 8/16
 *MHz crystal/resonator need to be removed) to provide an asynchronous clock
 *source to Timer 2. Please take note that Timer 2 is also used by the Arduino
 *core for PWM operation. Please refer to wiring.c for explanation. Removal of
 *the external 8/16 MHz crystal/resonator requires the microcontroller to run on
 *its internal RC oscillator which is not so accurate for time critical
 *operation.
 *
 * Argument  	Description
 * =========  	===========
 ...
 *			   is different from the power standby state as it has
 *the capability to run Timer 2 asynchronously. Not implemented on Atmega88 and
 *Atmega168.
 *
 * Argument  	Description
 * =========  	===========
...
void LowPowerClass::powerExtStandby(period_t period, adc_t adc, bod_t bod,
  ...
  if (period != SLEEP_FOREVER) {
    ...
    defined(__AVR_ATmega168__) // SLEEP_MODE_EXT_STANDBY not implemented on
                               // Atmega88 / Atmega168
#else
  if (bod == BOD_OFF) {
#if defined(__AVR_ATmega328P__) || defined(__AVR_ATmega168P__)
 ...
 *		       required to allow automatic WDIF and WDIE bit clearance
 *in hardware.
 *
 *******************************************************************************/
ISR(WDT_vect) {
 ...
 * 			   peripheral management (disabling and shutting down) to
 *achieve the desired current consumption.
 *
 * Argument  	Description
 * =========  	===========
